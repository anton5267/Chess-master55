$(function () {
    ...
    let selectedBoardTheme = getStoredValue(storageKeys.boardTheme, "classic", boardThemes);
    let selectedPieceTheme = getStoredValue(storageKeys.pieceTheme, "wikipedia", pieceThemes);
    ...
    connection.on("Start", function (game) {
        ...
        elements.whiteName.textContent = playerOneName;
        elements.blackName.textContent = playerTwoName;
    ...
    function applyBoardTheme(theme) {
        let themeClass = boardThemes[theme] || boardThemes.classic;
    ...
    function createOrRebuildBoard() {
        ...
        config.pieceTheme = pieceThemes[selectedPieceTheme] || pieceThemes.wikipedia;
        ...
        if (name !== "") {
            connection.invoke("CreateRoom", name)
                .then((player) => {
                    ...
                    elements.blackRating.textContent = "N/A";
                    elements.statusText.style.color = "red";
    ...
    elements.boardThemeSelect.value = selectedBoardTheme;
    elements.pieceThemeSelect.value = selectedPieceTheme;

    elements.boardThemeSelect.addEventListener("change", function (e) {
        selectedBoardTheme = e.target.value;
        applyBoardTheme(selectedBoardTheme);
        storeValue(storageKeys.boardTheme, selectedBoardTheme);
    });

    elements.pieceThemeSelect.addEventListener("change", function (e) {
        selectedPieceTheme = e.target.value;
        storeValue(storageKeys.pieceTheme, selectedPieceTheme);
        createOrRebuildBoard();
    });

    var config = {
        pieceTheme: pieceThemes[selectedPieceTheme],
        draggable: true,
